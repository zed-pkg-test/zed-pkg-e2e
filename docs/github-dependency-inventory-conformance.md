# GitHub dependency-inventory conformance (DEN-2957, DEN-2996)
...
A flat `.zpkg.lock` contributes exact package selections to the top-level `pins` evidence collection. A flat closure does not encode parent-child relationships, so pins are marked `topological: false` and never become graph nodes or edges. When one unique pin matches an already-proven direct `.zpkg.toml` declaration, that declaration edge may carry the selected version, artifact digest, optional VCS commit, and lock provenance. Unmatched/transitive pins remain evidence only.

`.zpkg.toml`, `.gitmodules`, `flake.nix`, `flake.lock`, and governed Nix `sources.json` files contribute typed source edges where the relationship is actually present in source. Only DEN-2865's shared resolver builder may claim an exact `zpkg/dependency-graph/v1` resolution.
...
- repository/package nodes and typed, proven topology edges;
- exact lock-pin evidence with package coordinate, selected version, artifact SHA-256, optional VCS commit, source package/repository, and immutable provenance;
- provenance containing repository commit, manifest kind, path, blob SHA, and line or JSON pointer where available;
- SCCs, canonical cycle sets, and dependency-first waves over topology only;
...
The JSON writer sorts keys and all semantically unordered collections. DOT and Mermaid use hash-derived identifiers and format-specific label escaping. Lock pins render as disconnected, explicitly labeled evidence nodes; they never receive or contribute dependency arrows. Golden outputs are regenerated independently on Linux, macOS, and Windows and compared byte-for-byte.
...
11. Flat lock entries are immutable evidence, not topology. They are excluded from nodes, edges, SCCs, cycles, and dependency-first waves. Multiple distinct selections for one coordinate remain explicit contradictions rather than being collapsed.
...
- a flat lock closure whose transitive `shared` selection must not fabricate `app -> shared`;
- direct declared-edge selection annotation, optional VCS commit evidence, conflicting/duplicate pins, and invalid artifact digests;
- Git submodule gitlinks, flake declarations/locks, and Nix source pins;
...
After the test-org conformance PRs are green, the production `zed-pkg/zed-cli` integration should add the modular `zed graph github` command and port this behavior without editing the merged DEN-2864 contract paths or introducing a second package resolver. The test fixture and goldens remain the cross-language conformance authority until the shared Rust builder supersedes them explicitly. Workspace/member manifest discovery is tracked independently in DEN-2997.
