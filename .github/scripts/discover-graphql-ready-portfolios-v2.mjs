  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const response = await fetch('https://api.github.com/graphql', {
      ...
      headers: {
        ...
        'User-Agent': 'graphql-ready-portfolio-discovery-v2',
      },
...
const searchQuery = `
  query ReadyPortfolios($search: String!) {
    ...
    search(query: $search, type: ISSUE, first: 100) {
      ...
      nodes {
        ... on PullRequest {
          id number title body createdAt state isDraft headRefName headRefOid baseRefName mergeable mergeStateStatus
          ...
          repository { nameWithOwner isPrivate isArchived viewerPermission defaultBranchRef { name } }
        }
      }
    }
...
const searchResult = await graphql(searchQuery, { search });
...
for (const entry of inspected.filter((value) => value.reasons.length === 0)) {
  const pr = entry.pr;
...
for (const name of ['FLEET_GH_TOKEN', 'GH_TOKEN', 'GITHUB_TOKEN']) delete sanitizedEnv[name];
...
for (let index = 0; index < selected.length; index += 1) {
  ...
  try {
    const cloneResult = spawnSync('git', ['clone', '--no-checkout', '--filter=blob:none', '--single-branch', '--branch', pr.headRefName, `https://github.com/${repo}.git`, clone], {
      env: sanitizedEnv, encoding: 'utf8', timeout: 180000, maxBuffer: 8 * 1024 * 1024,
    });
    ...
    const archive = spawnSync('git', ['-C', clone, 'archive', '--format=tar.gz', `--prefix=${repoName}/`, pr.headRefOid], {
      env: sanitizedEnv, stdio: ['ignore', file, 'pipe'], timeout: 120000, maxBuffer: 8 * 1024 * 1024,
    });
    fs.closeSync(file);
    ...
    manifest.push({
      repo, number: pr.number, node_id: pr.id, title: pr.title,
...
fs.writeFileSync(path.join(output, 'manifest.json'), JSON.stringify({
  schema_version: 1, generated_at: new Date().toISOString(), search_count: data.search.issueCount,
  selected_count: selected.length, entries: manifest, clone_failures: cloneFailures,
}, null, 2) + '\n');
console.log(`GRAPHQL_PORTFOLIO_DISCOVERY_V2 ${JSON.stringify({ searched: data.search.issueCount, inspected: inspected.length, eligible: eligible.length, selected: selected.length, archived: manifest.length, clone_failures: cloneFailures.length, graphql_remaining: data.rateLimit.remaining })}`);
